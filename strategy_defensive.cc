...
void Defensive::execute(CompPlayer *c_player){
	...
	for (unsigned int i = 0; i < c_player->get_owned_territories().size(); i++){
		ok_to_attack = true;
		if (!c_player->get_owned_territories()[i]->CanAttack() || c_player->get_owned_territories()[i] == NULL)
			continue;
		std::vector<Territory*> attackable = c_player->get_owned_territories()[i]->GetAttackableTerritories(c_player);
		...
		for (j = 0; j < attackable.size() - 1; j++){
			if (c_player->get_owned_territories()[i]->get_num_units() < (attackable[j]->get_num_units() + 4)){
				ok_to_attack = false; // at least one adjacent enemy country that doesn't 
				// meet its criteria has been found, so it won't attack
			}
		}
		if (ok_to_attack){
			std::cout << "\n" << c_player->get_owned_territories()[i]->get_name() << " attacks "
				<< attackable[0]->get_name() << " (Player " << attackable[0]->get_owner()->get_id()
				<< ")!" << std::endl;
			Battle::SingleBattle(c_player->get_owned_territories()[i], attackable[0]);
			has_attacked = true;
		}
	}
	if (!has_attacked){
