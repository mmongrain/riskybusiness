...
void Aggressive::execute(CompPlayer *c_player){
	...
	for (unsigned int i = 0; i < c_player->get_owned_territories().size(); i++){
		...
		for (unsigned int j = 0; j < attackable.size(); j++){
			...
			if (c_player->get_owned_territories()[i]->get_owner() != (attackable)[j]->get_owner()
				&& c_player->get_owned_territories()[i]->get_num_units() > attackable[j]->get_num_units()){
				std::cout << "\n" << c_player->get_owned_territories()[i]->get_name() << " attacks "
					<< attackable[j]->get_name() << " (Player " << attackable[j]->get_owner()->get_id()
					<< ")!" << std::endl;
				Battle::SingleBattle(c_player->get_owned_territories()[i], (attackable)[j]);
			}
		}
	}
}
